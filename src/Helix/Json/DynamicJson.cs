...
namespace Helix.Json
{
    ...
    public class DynamicJson : DynamicObject
    {
        ...
        {
            if(json == null )
                throw new Exception("JSON string cannot be null.");

            json = json.StartsWith("{") && json.EndsWith("}") ? json : "{" + json + "}";
        ...
        public override bool TryGetIndex(GetIndexBinder binder, object[] indexes, out object result)
        {
            ...
            {
                //catch exceptions for missing bindings @ run-time.
                result = null;
                return false;
            }
        }
        ...
        public static object TryGetValue(dynamic item, string nameOfItem)
        {
            try
            {
                return item[nameOfItem] ?? null;
            }
            catch
            {
                return null;
